...
#[test]
fn extract_hex6() {
    assert_eq!(extract_color("#E53935"), Some(ColorValue::Rgba(0xE5, 0x39, 0x35, 255)));
}

#[test]
fn extract_hex3() {
    assert_eq!(extract_color("#f00"), Some(ColorValue::Rgba(0xff, 0x00, 0x00, 255)));
}

#[test]
fn extract_hex8_carries_alpha() {
    // #rrggbbaa — the trailing pair is the alpha channel.
    assert_eq!(extract_color("#3482e380"), Some(ColorValue::Rgba(0x34, 0x82, 0xe3, 0x80)));
}

#[test]
fn extract_hex4_shorthand_alpha() {
    // #rgba — each nibble doubled, so `8` → `0x88`.
    assert_eq!(extract_color("#f008"), Some(ColorValue::Rgba(0xff, 0x00, 0x00, 0x88)));
}

#[test]
fn extract_rgb() {
    assert_eq!(extract_color("rgb(255, 0, 128)"), Some(ColorValue::Rgba(255, 0, 128, 255)));
}

#[test]
fn extract_rgba_float_alpha() {
    // CSS alpha is a 0..1 float; 0.5 → 128 (rounded).
    assert_eq!(extract_color("rgba(255, 0, 0, 0.5)"), Some(ColorValue::Rgba(255, 0, 0, 128)));
}

#[test]
fn extract_hsl_primary_red() {
    // hsl(0, 100%, 50%) is pure red.
    assert_eq!(extract_color("hsl(0, 100%, 50%)"), Some(ColorValue::Rgba(255, 0, 0, 255)));
}

#[test]
fn extract_hsl_grey_when_unsaturated() {
    // 0% saturation collapses to a grey at the given lightness.
    assert_eq!(extract_color("hsl(210, 0%, 50%)"), Some(ColorValue::Rgba(128, 128, 128, 255)));
}

#[test]
fn extract_transparent_keyword() {
    assert_eq!(extract_color("transparent"), Some(ColorValue::Rgba(0, 0, 0, 0)));
}

#[test]
fn extract_extended_named_color() {
    // rebeccapurple is part of the full 148-colour set, not the old 16.
    assert_eq!(extract_color("rebeccapurple").map(|c| c.to_rgb()), Some((102, 51, 153)));
}
...
fn to_hex_string_normalises() {
    assert_eq!(ColorValue::Rgba(0x3A, 0x82, 0xE3, 255).to_hex_string(), "#3a82e3");
    assert_eq!(ColorValue::Named("blue".into()).to_hex_string(), "#0000ff");
}

#[test]
fn to_hex_string_includes_alpha_when_translucent() {
    assert_eq!(ColorValue::Rgba(0x34, 0x82, 0xe3, 0x80).to_hex_string(), "#3482e380");
}
...
fn two_sections() {
    ...
    assert_eq!(doc.sections[1].swatches[0].color, ColorValue::Rgba(30, 30, 30, 255));
}
...
fn last_color_token_wins_in_item() {
    ...
    let doc = parse(md);
    assert_eq!(doc.sections[0].swatches[0].color, ColorValue::Rgba(0x00, 0xff, 0x00, 255));
}
